...
def Init(p1,p2,p3,p4):
    ...
    grid[0:20, 33:37, 0] = oNN.Object(permittivity=torch.ones([20, 4, 1], device=device)*1.8 + 1, name="wg1")
    grid[0:20, 43:47, 0] = oNN.Object(permittivity=torch.ones([20, 4, 1], device=device)*1.8 + 1, name="wg2")
    grid[0:20, 53:57, 0] = oNN.Object(permittivity=torch.ones([20, 4, 1], device=device)*1.8 + 1, name="wg3")
    grid[0:20, 63:67, 0] = oNN.Object(permittivity=torch.ones([20, 4, 1], device=device)*1.8 + 1, name="wg4")

    #探测器位置
    for i in range(0, 10):
        grid[80, 31 + 4 * i : 33 + 4 * i, 0] = oNN.LineDetector(name="detector" + str(i))
        grid[80:100, 31 + 4 * i : 33 + 4 * i, 0] = oNN.Object(permittivity=torch.ones([20, 2, 1], device=device)*1.8 + 1, name="op" + str(i))
    return grid
...
class OpTorch(torch.nn.Module):
    ...
    def forward(self, time, x_in, blur=False, Quant=False):
        ...
        grid[self.x_min:self.x_max, self.y_min:self.y_max, 0] = oNN.Object(permittivity=self.Permittivity*1.8+1, name="object")

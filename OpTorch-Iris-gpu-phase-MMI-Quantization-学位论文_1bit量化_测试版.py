...
X_test = X_shuffled[0:24]
Y_test = y_shuffled[0:24]

X_train, X_test, y_train, y_test = train_test_split(X_test, Y_test, test_size=0.8, random_state=42)
    ...
    for i in tqdm(range(0, num_samples, batch_size)):
        ...
        for j in range(0, X_batch.shape[0]):
            ...
            for det_index in [1,5,9]: # 选择哪几个探测器作为输出，把150个timestep求和了
                ...
                # source3 = []
                # 查看光源波形
                # for x in sim.detectors[10].detector_values()["E"]:
                #     # source.append()
                #     source0.append(x[0,2].item())
                #     source1.append(x[1,2].item())
                #     source2.append(x[2,2].item())
                #     source3.append(x[3,2].item())
                #     print(x)
                # print('sim:', sim)
                # for x in sim.detectors[det_index].detector_values()["E"]:
                #     flag = flag + 1
                #     Ex.append(x[0,2].item())
                #     Ey.append(x[1,2].item())
                #     E.append(x[0,2].item()**2+x[1,2].item()**2)
                # if(i==0 and j==0):
                #     plt.plot(Ex)
                #     plt.title('Ex')
                #     plt.savefig('Ex.png')
                ...
                #     plt.plot(source0)
                #     plt.title('source0')
                #     plt.savefig('source0.png')
                #     plt.close()                    
                #     plt.plot(source1)
                #     plt.title('source1')
                #     plt.savefig('source1.png')
                #     plt.close()                    
                #     plt.plot(source2)
                #     plt.title('source2')
                ...
                #     plt.plot(source3)
                #     plt.title('source3')
                #     plt.savefig('source3.png')
                #     plt.close()
                #     print('all right')
                    # print(det_index, flag, x**2)
                    # if(i==0 and j==0):
                    #     print(det_index, flag, (x**2).mean(dim=0)[-1])
                    #     print('output:', output)
...
for i in tqdm(range(X_scaled.shape[0])):
    ...
    for det_index in [1,5,9]: # 选择哪几个探测器作为输出
        ...
        for x in sim.detectors[det_index].detector_values()["E"]:
            ...
            print('det_index:', int(det_index/4), x)
            flag = flag + 1
        output=torch.cat([output,sum([(x**2).mean(dim=0)[-1] for x in sim.detectors[det_index].detector_values()["E"]]).unsqueeze(dim=0)],dim=0)
    output = output/torch.sum(output).reshape(1,-1)
    # 获取预测结果

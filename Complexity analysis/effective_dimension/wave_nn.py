...
import skimage
import numpy as np
from .utils import to_tensor, _laplacian, normalize_power
...
class WaveSource(torch.nn.Module):
    ...
    def forward(self, Y, X, dt=1.0):
        ...
        X_expanded = torch.zeros(Y.size()).detach().to(X.device)
        X_expanded[:, self.x, self.y] = X
        # print('X_expanded:', type(X_expanded), X_expanded.shape)	#sjk Tensor [batch_size, 150, 100]
        # print('X:', type(X), X.shape)  # sjk Tensor	[6]
        # print('self.x:', self.x, 'self.y', self.y)	#x:tensor(40)  y:tensor(50) coordinate of source
...
class WaveGeometry(torch.nn.Module):
    def __init__(self, domain_shape: Tuple, h: float, c0: float, c1: float, abs_N: int = 20, abs_sig: float = 11,
        ...
        self.register_buffer("abs_N", to_tensor(abs_N))
        self.register_buffer("abs_sig", to_tensor(abs_sig))
        self.register_buffer("abs_p", to_tensor(abs_p))
...
class WaveGeometryFreeForm(WaveGeometry):
    def __init__(self, domain_shape: Tuple, h: float, c0: float, c1: float, abs_N: int = 20, abs_sig: float = 11,
        ...
        rr, cc = ellipse(blur_radius, blur_radius, blur_radius+1, blur_radius+1)
...
class WaveRNN(torch.nn.Module):
    ...
    def forward(self, x, output_fields=False):
        ...
        # First dim is batch
        batch_size = x.shape[0]			#sjk batch_size = 6 or 2  x is [6, 1000] or [2, 1000]

        # Init hidden states
        hidden_state_shape = (batch_size,) + self.cell.geom.domain_shape	#sjk domain_shape = (150, 100)
        h1 = torch.zeros(hidden_state_shape, device=x.device)					#(batch_size, 150, 100)
        h2 = torch.zeros(hidden_state_shape, device=x.device)					#(batch_size, 150, 100)
        y_all = []
        ...
        # rnn forward for Iris dataset
        x_tuple = x.chunk(x.size(1), dim=1)
        # print("x_tuple", x_tuple)
        for i in range(0, 300):
            h1, h2 = self.cell(h1, h2, c, rho)
            source_num = 0
            for source in self.sources:
                h1 = source(h1, x_tuple[source_num].squeeze(-1))
                # print("h1",x_tuple[source_num].squeeze(-1))
        ...
        # print('y_all', type(y_all), y_all[0].shape)  # sjk   list，each element is Tensor sized [batch_size, 3]
        y = torch.stack(y_all, dim=1)
        y = y.sum(dim=1)
        # y = torch.nn.functional.softmax(y, dim=1)
        # print('y', type(y), y.shape)  # sjk	Tensor sized [batch_size, 1000, 3]

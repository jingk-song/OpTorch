...
class EffectiveDimension:
    def __init__(self, model, num_thetas, num_inputs):
        ...
        rep_range = np.tile(np.array([num_inputs]), num_thetas)     # np.tile 对数组进行重复
        # print("rep_range:{}".format(rep_range.shape))   # rep_range:(100,)
        params = np.random.uniform(self.model.thetamin, self.model.thetamax, size=(self.num_thetas, model.d))
        # print("params:{}".format(params.shape))     # params:(100, 40)
        self.params = np.repeat(params, repeats=rep_range, axis=0)
        # print("self.params:{}".format(self.params.shape))   # self.params:(10000, 40)
    ...
    def get_fhat(self):
        ...
        # fisher矩阵的大小规模是 np((len(gradients), self.d, self.d)),这里average是对(d, d)每个元素求平均值
        ...
        return f_hat, fisher_trace, grads, output, fishers
